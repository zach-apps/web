<template>
  <div id='recipe-viewer' style="height:100%;">
    <router-link to="/recipes" id="back-button">« BACK</router-link>
      ...
      <div class="description">
        ...
        <div>
          ...
          <p><span class="bold">Time: </span>{{time}}</p>
            ...
            <div class="starbox">
              <p><span class="bold">Rating: </span><span style="color:var(--bright);font-size:1.3em" :key="i" v-for="i in recipe.rating">★</span><span style="color:var(--dark);font-size:1.3em" v-for="i in 5 - recipe.rating" :key="i">☆</span> ({{formatRatingCount}})</p>
                </div>
        </div>
        <div id="imagebox">
...
export default {
  ...
  computed: {
    ...
    recipe() {
      let recipes = this.$root.$data.recipes;
      console.log(recipes[this.recipe_index])
...
.content {
  width: var(--recipes-width);
  position: absolute;
  margin: 0 calc((100vw - var(--recipes-width)) / 2);
  left: 0;
}

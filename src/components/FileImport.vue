<template lang="html">
  <div>
    ...
    <div class="frontpage" v-if="front_page">
      ...
      <p>
        Request your data from Spotify at the bottom of your account's
        <a target="_blank" href="https://www.spotify.com/us/account/privacy/"
          >Privacy Settings</a
        >
        page (it takes up to 30 days to gather streaming history) After 
        you get an email to download, click
        <a class="bold" v-on:click="change_page">here.</a>
      </p>
      <p>
        For an explanation of this project and its process, click
        <router-link to="/projects/spotify_streamgraph_explanation"
          >here.</router-link
        >
      </p>
    </div>
    <div class="backpage" v-if="!front_page">
      <div class="import_data" v-if="import_pending">
        ...
        <p>
          Your zipped data file will include a number of
          <span class="mono">StreamingHistoryJSON</span> files starting from
          <span class="mono">0</span> and counting up. Add each of these files
          sequentially, then press create to view your streamgraph.<br />
        </p>
        <p>
          <a class="bold" @click="change_page"
            >[how to download spotify data]</a
          >
        </p>
        <hr />
        <div class="left">
          <div class="box">
            ...
            <button
              disabled
              :style="hover_style(file_selected)"
              id="import"
              @click="
                import_file();
                change_file();
            ...
            <button
              id="gogogo"
              :style="hover_style(file_added)"
              @click="group_by_artist"
            >
              create
            </button>
      ...
      <div
        class="vizpage"
        id="vizpage"
        v-bind:style="import_pending ? 'display: none;' : 'display: unset;'"
        style="width:100%"
      >
        <h2>top artists</h2>
        ...
        <div
          v-bind:style="cardinal_switch1 ? 'display: unset;' : 'display: none;'"
          id="streamgraph"
        ></div>
        <div
          v-bind:style="step_switch1 ? 'display: unset;' : 'display: none;'"
          id="streamgraph2"
        ></div>
        <div
          v-bind:style="point_switch1 ? 'display: unset;' : 'display: none;'"
          id="streamgraph3"
        ></div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    change_file() {
      ...
      document.getElementById("import").disabled = !document.getElementById(
        "import"
      ).disabled;
    },
    change_page() {
    ...
    import_file() {
      ...
      fr.onload = e => {
        ...
        if (this.file_added == false) {
          ...
          this.$root.$data.importedJSON = this.$root.$data.importedJSON.concat(
            result
          );
        }
    ...
    group_by_artist() {
      ...
      this.$root.$data.artist_list = _.mapValues(
        _.groupBy(json, "artistName"),
        x => x.map(y => _.omit(y, "artistName"))
      );
      this.sumPlayTime();
    },
    ...
    sumPlayTime() {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        coolobj[newArray[i].artistName] = newArray[i].minutesListened.toFixed(
          2
        );
    ...
    groupByWeek(items) {
      //let arrayItems = []
      let objectItems = _.groupBy(items, result =>
        moment(result["endTime"], "YYYY-MM-DD HH:mm").startOf("isoWeek")
      );
      let newArray = [];
      let weekArray = [];
      ...
      for (let item in objectItems) {
        ...
        groupedWeekArray = _.mapValues(_.groupBy(weekArray, "artistName"), x =>
          x.map(y => _.omit(y, "artistName"))
        );
      ...
      let groupedWeekObj = _.mapValues(_.groupBy(newArray, "week"), x =>
        x.map(y => _.omit(y, "week"))
      );
    ...
    createStreamgraph() {
      let week_max = 0.7 * this.$root.$data.week_max;
      ...
      var xAxis = svg
        ...
        .call(g => g.selectAll(".tick line").style("stroke", "black"))
        .call(g =>
          g
            .selectAll(".tick text")
            .style("fill", "black")
